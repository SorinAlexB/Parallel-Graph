...
void in_sync_threadp(os_threadpool_t *tp)
{
	pthread_mutex_init(&(tp->mutex), NULL);
	pthread_cond_init(&(tp->cond), NULL);
	pthread_mutex_init(&tp->xmutex, NULL);
	tp->ct = 0;
	tp->x = 0;
}

void clean(os_threadpool_t *tp)
{
	pthread_mutex_destroy(&(tp->mutex));
	pthread_mutex_destroy(&(tp->xmutex));
...
void verif_first_thread(os_threadpool_t *tp)
{
...
void verif_threads(os_threadpool_t *tp)
{
